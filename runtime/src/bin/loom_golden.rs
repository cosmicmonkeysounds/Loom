...
//! `core/test` checks the TS engine against:
//!
//! - `sources.json` — `{ relPath: source }` (so the TS test is
//!   self-contained; it never reads the examples dir at test time).
//! - `diagnostics.json` — `{ relPath: JsDiagnostic[] }`, the exact
//!   shape the former `wasm` crate's `JsDiagnostic` emitted.
//! - `ast.json` — `{ relPath: <serde LoomFile> }`.
//! - `apply.json` — `{ relPath: [ {op, args, ok, output} ] }`, a
//!   deterministic script of `apply_*` ops with byte-exact expected
//!   output (the round-trip identity guard for the edit API).
//!
