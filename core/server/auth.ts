...
/** A fresh, random, speakable passcode (e.g. `K7PQ3M`). Rejection-sampled
 *  so every alphabet character is equally likely (no `% 31` modulo bias). */
export function makePass(): string {
  // Largest multiple of the alphabet size below 256 — bytes past it re-roll.
  const bound = 256 - (256 % ALPHABET.length);
  let out = "";
  while (out.length < LEN) {
    for (const byte of randomBytes(LEN)) {
      if (byte < bound && out.length < LEN) out += ALPHABET[byte % ALPHABET.length];
    }
  }
  return out;
}
