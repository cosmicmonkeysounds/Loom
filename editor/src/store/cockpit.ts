...
export interface CockpitState {
  ...
  /** Spawn a test persona to act as — a local guest on the Sim source, a
   *  journaled `/api/mod/persona` guest on a live/preview event (the
   *  shared-rehearsal path: each co-writer puppets their own). */
  addPersona(name?: string): Promise<void>
  /** Write any world variable (the World browser's inline editing) —
   *  `Sim.setVar` locally, the journaled `/api/mod/var` on an event. */
