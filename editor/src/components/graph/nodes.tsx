...
export type BeatNodeData = {
  ...
  visits?: number
  isCurrent?: boolean
  /** Participants whose live story position is this beat (display names). */
  occupants?: string[]
...
export const BeatNode = memo(function BeatNode({ data, selected }: NodeProps<BeatFlowNode>) {
  const { beat, blocks, onToggleExpand, visits, isCurrent, occupants } = data
      ...
      <div className="px-2.5 pt-2">
        ...
        {occupants !== undefined && occupants.length > 0 && (
          <div
            ...
            title={`Here now: ${occupants.join(', ')}`}
            data-testid={`graph-beat-occupants-${beat.key}`}
          >
            {occupants.slice(0, 4).map((name) => (
              <span
                key={name}
                className="rounded-full border border-emerald-500/40 bg-emerald-500/15 px-1.5 text-[9px] leading-4 text-emerald-200"
              >
                ⦿ {name}
              </span>
            ))}
            {occupants.length > 4 && (
              <span className="rounded-full bg-zinc-800 px-1.5 text-[9px] leading-4 text-zinc-400">
                +{occupants.length - 4}
              </span>
            )}
          </div>
