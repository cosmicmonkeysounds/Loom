...
function Canvas({ variant }: { variant: 'edit' | 'run' }) {
  ...
  // Destructured so the memo below reads plain values (the compiler lint
  // reads a bare `runtime.current` access as a React ref).
  const { visits: rtVisits, current: rtCurrent, positions: rtPositions } = runtime
  ...
  const nodes = useMemo(() => {
    ...
    const currentFile = graph.files.find(
      (f) => rtCurrent !== null && f.beats.includes(rtCurrent),
    )
    const currentFilePath = currentFile !== undefined ? pathForUri(currentFile.uri) : null
    // Live participant positions, grouped per beat — occupant chips.
    const occupantsByBeat = new Map<string, string[]>()
    for (const pos of Object.values(rtPositions)) {
      const list = occupantsByBeat.get(pos.beat) ?? []
      list.push(pos.label)
      occupantsByBeat.set(pos.beat, list)
    }
    return positioned.nodes.map((n) => {
      ...
      const decorated: Node = {
        ...
        data: {
          ...
          ...(isBeat
            ? {
                visits: rtVisits[n.id],
                isCurrent: rtCurrent === n.id,
                occupants: occupantsByBeat.get(n.id),
  ...
  }, [positioned, selected, rtVisits, rtCurrent, rtPositions, graph, editable, beatEditable, editingNode, editingBlock, commitInlineEdit, commitBlockEdit, openBlockMenu, setEditing, setEditingBlock, toggleExpanded, toggleFileCollapsed, dimmedIds])
