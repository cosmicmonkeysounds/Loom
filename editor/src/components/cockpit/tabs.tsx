...
// ---------------------------------------------------------------------------
// World — the live state browser: vitals, factions, and every variable in
// the world (globals · story clock · per-guest · cast internals), searchable
// and updating with each snapshot. Locations + occupancy live on the Stage.
// ---------------------------------------------------------------------------
...
/** One collapsible variable group as a name → value table. */
function VarGroup({ group, forceOpen, defaultOpen }: { group: WorldGroup; forceOpen: boolean; defaultOpen?: boolean }) {
  const [open, setOpen] = useState(defaultOpen ?? false)
  const show = forceOpen || open
  return (
    <div className="rounded-lg border border-zinc-800" data-testid={`world-group-${group.key}`}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center gap-2 px-2 py-1.5 text-left text-sm hover:bg-zinc-900/60"
      >
        <Chevron open={show} />
        <span className="min-w-0 flex-1 truncate text-zinc-200">{group.title}</span>
        <span className="shrink-0 text-[10px] text-zinc-600">{group.entries.length}</span>
      </button>
      {show && <VarTable rows={group.entries} className="w-full border-t border-zinc-800 text-xs" />}
    </div>
  )
}
...
function WorldBrowser() {
  ...
  const grouped = useMemo(
    () => groupWorld(world, roster, cast, factions, locations, query),
    [world, roster, cast, factions, locations, query],
  )
  // A live search forces every surviving group open — you're hunting a value.
  ...
  return (
    <div className="h-full overflow-auto p-3">
      ...
      <div className="flex items-baseline gap-3 pt-4">
        <span className="text-[10px] uppercase tracking-widest text-zinc-500">Variables</span>
        <span className="text-[10px] text-zinc-600">double-click a value to edit it live</span>
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="filter paths + values… (e.g. suspicion, score, Time)"
          className="min-w-0 flex-1 rounded border border-zinc-800 bg-zinc-950 px-2 py-1 text-xs text-zinc-200 outline-none focus:border-indigo-500"
          data-testid="world-var-filter"
        />
      </div>
      {world.length === 0 ? (
        ...
        <div className="flex flex-col gap-1 pt-2">
          ...
          {grouped.clock && <VarGroup group={grouped.clock} forceOpen={searching} />}
          {grouped.guests.map((g) => (
            <VarGroup key={`g:${g.key}`} group={g} forceOpen={searching} defaultOpen />
          ))}
          {grouped.cast.map((g) => (
            <VarGroup key={`c:${g.key}`} group={g} forceOpen={searching} />
          ))}
          {grouped.world.map((g) => (
            <VarGroup key={`w:${g.key}`} group={g} forceOpen={searching} />
          ))}
        </div>
      )}
    </div>
