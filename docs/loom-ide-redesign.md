...
> ⚠️ **Historical.** The Phases below describe the earlier
> dockview-based, five-mode editor. The shipped shell is the
> **three-mode Writing / Run / Deploy studio** (Writing `⌘1`, Run `⌘2`
> with a Sim ⇄ Live cockpit, Deploy `⌘3`); the wasm engine and its
> in-editor runtime surfaces (Transcript / Ledger / Timeline / Cast /
> Booth, multi-head play) were removed — Run mode's Sim source is the
> local-play successor. This document is kept for design history.
...
```
pnpm --filter loom-app dev      # editor (HMR): http://127.0.0.1:5173
cargo run -p loom-server --bin loom-relayd -- --cors permissive
                                # relay (API+WS): http://127.0.0.1:7878
```

(The `prism loom dev` one-shot was the Prism monorepo CLI's wrapper
around these two processes; that CLI does not exist here, and the Rust
`loom-server` crate depends on `prism-core`, so it builds only inside
the Prism monorepo.) The wrapper:
...
Useful flags (of the retired wrapper):
...
```
pnpm --filter loom-app build            # vite build → editor/dist
cargo run -p loom-server --bin loom-relayd -- \
  --editor-dist editor/dist             # boot → http://127.0.0.1:7878
```

`loom-relayd` flags: `--bind 0.0.0.0:7878`, `--editor-dist <path>`,
`--cors permissive`. (Again: the `loom-server` crate builds only
inside the Prism monorepo.)

Local debugger (no relay, no editor):

```
cargo build -p loom-runtime --bin loom-play
python simulator/simulator.py examples/saltmere
```
...
cd editor && npx tsc -b             # type-check
cd editor && pnpm lint              # 0 errors, 0 warnings
cd editor && pnpm build             # vite production build
```
...
1. **Runtime: snapshot/restore + multi-head driver** (`runtime`). Add `Playhead::snapshot/restore`, multi-head dispatch in `loom_play.rs`, mesh tests. No editor change.
...
Phase 1 ships when:
- `cargo test -p loom-runtime` passes including the new tests.
