...
> ⚠️ **Superseded.** This roadmap assumed a **WASM engine + Rust
> `loom-relayd`** deployment. Both are gone: the engine is now native
> **TypeScript** (`@loom/core`, no wasm) and live events run on the TS
> event server. For actual deployment see
> [`loom-deployment.md`](./loom-deployment.md); kept for design history.
> Paths and commands in the body refer to the old prism-root monorepo
> layout (`packages/loom/...`, the `prism` CLI) and are kept verbatim as
> history; in this repo the Rust `server` crate builds only inside the
> Prism monorepo.
