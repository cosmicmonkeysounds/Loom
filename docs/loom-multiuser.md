...
> ⚠️ **Superseded for deployment.** This describes the older **Rust**
> `loom-server` / `loom-relayd` backbone (axum + WebSocket sync, port
> 7878, wasm client). The shipped live stack is the **TypeScript**
> `@loom/core` event server (SSE/REST, port 7000, BetterAuth + Postgres)
> + the `loom-play` app. **To deploy Loom, use
> [`loom-deployment.md`](./loom-deployment.md).** This document is kept
> for the Rust backbone's design history only. Paths and commands in the
> body refer to the old prism-root monorepo layout (`packages/loom/...`,
> the `prism` CLI) and are kept verbatim as history; in this repo the
> Rust `server` crate builds only inside the Prism monorepo.
