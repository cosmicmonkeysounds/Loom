...
> existing `core/` model. The TypeScript engine (`core/`) is the maintained
> implementation; the Rust mirror is **not** updated for these slices and is
> deprioritized for now.
> Target file: `docs/loom-functional-redesign.md`.
>
...
> Lowers entirely into the existing `core/` model (the maintained TypeScript
> engine; the Rust mirror is not updated for these slices and is deprioritized).
> Continues the section numbering of Part I (§1–§8). **Depends on
